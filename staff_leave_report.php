                    ...
                    <div class="grid paddin10 no-padding-top  shadow bg-white">
                        <div class="row">
                            <div class="cell padding10">
                               <h3 class="align-center fg-crimson uppercase">My Leaves Status</h3> 
                               <form method="POST">
                               <?php
                                    ...
                                    $leave = new Leaves($db);                                     
                                    $curr_user = $db->postElements($_SESSION['curr_staff_no']);                                 
                                    $leave->displayForStaff($curr_user);
                               ?>
                               </form>
                            </div>
                        </div>
...
if(isset($_POST['cancel_selection'])){
    ...
    $id =$_POST['cancel_selection'];
    $count= count($_POST['cancel_selection']);
    
    for($i=0;$i<$count ;$i++){
        
        $col=trim($id[$i]);
        $stmt="DELETE FROM staff_leave WHERE leave_code='".$col."'";
        $query = $db_conn->multi_query($stmt);
       	if($query){
       	    ?>
               <script>window.location="staff_leave_report.php";</script>
               <?php
       	}				
    ...
    //echo "Add successfull";
            echo "<script><!-- Error notify -->
                    $.Notify({
                        caption: 'Tip',
                        content: 'You can cancel or delete leave by doing the follwing: Select leave(s) first, then click  <Cancel Selected> button!',
                        type:'info',                                                                                              
                        keepOpen: false
                    });
                </script>";
